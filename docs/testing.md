...
- **`unit`** runs in plain Node with no Workers runtime. It covers the pure rules
  ...
  replay's filmstrip and stat lines, and the thinking-ellipsis predicate. These
  are the tests that can be reasoned about from the code alone, and they run
  fast because there is no platform underneath them.
