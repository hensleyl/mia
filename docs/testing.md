...
- **`unit`** runs in plain Node with no Workers runtime. It covers the pure rules
  ...
  seat geometry, the lobby table's rim occupancy, the showdown's verdict and beat
  arithmetic and the endgame replay's filmstrip and stat lines. These are the
  tests that can be reasoned about from the code alone, and they run fast because
