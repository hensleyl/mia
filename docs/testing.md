...
- **`unit`** runs in plain Node with no Workers runtime. It covers the pure rules
  ...
  replay's filmstrip and stat lines, the snapshot transitions that fire the
  optional delights, and the haptic patterns and preference parse. These are
  the tests that can be reasoned about from the code alone, and they run fast
